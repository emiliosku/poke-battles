...
class TestDebug:
    def test_orchestrator_snapshot_no_auth(self, client: TestClient) -> None:
        r = client.get("/debug/orchestrator")
        assert r.status_code == 200
        body = r.json()
        assert "uptime_s" in body
        assert body["showdown"]["started"] is False
        assert body["orchestrator"]["running"] is True
        assert body["orchestrator"]["max_concurrent"] >= 1
        assert "worker_count" in body["orchestrator"]

    def test_logs_tail_caps_param(self, client: TestClient) -> None:
        r = client.get("/debug/logs?tail=10")
        assert r.status_code == 200
        body = r.json()
        assert isinstance(body["logs_tail"], list)
        assert len(body["logs_tail"]) <= 10

    def test_logs_tail_rejects_out_of_range(self, client: TestClient) -> None:
        assert client.get("/debug/logs?tail=0").status_code == 422
        assert client.get("/debug/logs?tail=999").status_code == 422

    def test_logs_reflect_messages_emitted_into_app_handler(self, client: TestClient) -> None:
        import logging
        ...
        try:
            logger.info("debug-endpoint-marker-xyz")
            r = client.get("/debug/logs?tail=200")
            assert r.status_code == 200
            body = r.json()
            assert any("debug-endpoint-marker-xyz" in line["message"] for line in body["logs_tail"])
        finally:
